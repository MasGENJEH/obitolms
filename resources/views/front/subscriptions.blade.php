...
@section('title', 'Obito | Subscription')
    ...
    <main class="relative flex flex-1 h-full">
        ...
        <section id="subscriptions-list"
            ...
            <div id="list-container" class="flex flex-col gap-5 max-w-[800px] w-full">
                ...
                @foreach ($transactions as $subscription)
                    <div
                        ...
                        <div class="flex items-center flex-1 gap-[14px]">
                            ...
                            <div>
                                <p class="font-bold text-lg">{{ $subscription->pricing->name }}</p>
                                <p class="text-obito-text-secondary">{{ $subscription->pricing->duration }} months duration
                                </p>
                            </div>
                        </div>
                        <div class="flex flex-col w-[100px] shrink-0 gap-1">
                            ...
                            <p class="font-semibold text-sm">Rp
                                {{ number_format($subscription->sub_total_amount, 0, '', '.') }}</p>
                        </div>
                        <div class="flex flex-col w-[150px] shrink-0 gap-1">
                            ...
                            <p class="font-semibold text-sm">{{ $subscription->started_at->translatedFormat('d F Y') }}</p>
                        </div>
                        <div class="flex items-center justify-center w-[75px] shrink-0">
                            <span
                                class="font-bold text-xs badge w-fit rounded-full py-[6px] px-[10px] gap-[6px] {{ $subscription->ended_at > now() ? 'text-obito-green bg-obito-light-green' : 'text-obito-red bg-obito-light-red' }}">
                                {{ $subscription->ended_at > now() ? 'ACTIVE' : 'EXPIRED' }}
                            </span>

                        </div>
                        <a href="{{ route('dashboard.subscriptions.details', ['transaction' => $subscription->id]) }}"
